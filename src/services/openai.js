...
export async function generarRespuestaLegal(mensaje, options = {}) {
  ...
  const verifiedSourcesContext = sourcesContext
    ? `\n\nFUENTES OFICIALES DISPONIBLES PARA ESTA RESPUESTA\n${sourcesContext}\n\nUsa estas fuentes solo si son pertinentes para la consulta. Cuando menciones una sentencia o fuente de esta lista, agrega el enlace Markdown al final del mismo parrafo, por ejemplo: [Fuente oficial](https://...). Cuando la fuente tenga extracto util, incorpora una sintesis breve de ese extracto para sustentar el punto juridico, sin excederte ni copiar bloques largos. No cites como verificada una fuente que no aparezca aqui o que el usuario no haya aportado. Si las fuentes no son pertinentes, dilo y responde sin forzar citas.`
    : "";
