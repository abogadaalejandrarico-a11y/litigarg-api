...
- La falta de prueba sobre entrega previa no demuestra por si sola un apoderamiento. Si el relato tampoco describe quien tomo los bienes, de donde, cuando y mediante que conducta, no elijas hurto como hipotesis principal: concluye que los hechos son insuficientes para determinar el tipo y presenta hurto y abuso de confianza solo como alternativas condicionadas.
...
- No infieras parentescos a partir de apellidos o proximidad narrativa. Reproduce exactamente quien es familiar de quien solo cuando el documento lo diga con claridad; si la relacion es ambigua, marcala como dato por verificar.
...
async function buildSystemPrompt(options = {}) {
  ...
  return `${activeRules}${userContext}${activeConversationContext}${internalLibraryContext}${activeLearningContext}${responseGuidance}${uploadedDocumentContext}${verifiedSourcesContext}`;
}
